...
public class UnitServiceTests : BusinessIntegrationTest
{
    ...
    [Theory]
    [InlineData("Yas")]
    public void Add_throw_exception_when_unit_name_duplicated(string unitName)
    {
        var block1 = BlockFactory.Create("Block1", 2);
        Save(block1);
        var floor1 = FloorFactory.Create("Floor1", block1.Id,3);
        Save(floor1);
    ...
    public void Add_throw_exception_when_floor_has_max_number_of_units()
    {
        var block1 = BlockFactory.Create("Block1");
        Save(block1);
        var floor1 = FloorFactory.Create("Floor1",block1.Id,1);
        Save(floor1);
    ...
    public void Update_update_a_unit_properly()
    {
        ...
        var actual = ReadContext.Set<Entities.Units.Unit>().FirstOrDefault();
    ...
    [Theory]
    [InlineData(-1)]
    public void Update_throw_exception_when_floor_id_is_not_found(int invalidFloorId)
    {
        var dto = UnitFactory.AddUnitDto("Unit1");
    ...
    [Theory]
    [InlineData(-1)]
    public void Delete_throw_exception_if_unit_does_not_exist(int invalidUnitId)
    {
